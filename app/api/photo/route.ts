...
export async function POST(r:Request){
 ...
 try{const bytes=await r.arrayBuffer();if(bytes.byteLength>max)return Response.json({error:'حداکثر حجم عکس ۵ مگابایت است.'},{status:400});const a=new Uint8Array(bytes);const valid=(type==='image/jpeg'&&a[0]===255&&a[1]===216&&a[2]===255)||(type==='image/png'&&a[0]===137&&a[1]===80&&a[2]===78&&a[3]===71)||(type==='image/webp'&&String.fromCharCode(...a.slice(0,4))==='RIFF'&&String.fromCharCode(...a.slice(8,12))==='WEBP');if(!valid)return Response.json({error:'فایل تصویر معتبر نیست.'},{status:400});await bucket().put('profiles/'+id,bytes,{httpMetadata:{contentType:type}});return Response.json({url:'/api/photo?v='+Date.now()})}catch(e){console.error('Profile photo upload failed',e);return Response.json({error:'بارگذاری عکس انجام نشد؛ دوباره تلاش کنید.'},{status:503})}
}
